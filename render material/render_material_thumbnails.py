...
RESOLUTION = 256                 # width & height in pixels
FILE_FORMAT = 'PNG'
ENGINE = 'CYCLES'                # 'CYCLES' = reliable headless. 'BLENDER_EEVEE_NEXT' = faster, best run interactively
SAMPLES = 32                     # Cycles samples (lower = faster/noisier)
PLANE_SIZE = 2.0
BACKGROUND_COLOR = (0.05, 0.05, 0.05, 1.0)   # dark neutral backdrop
...
def build_render_scene():
    ...
    # Camera - slight angle looking down at the plane
    cam_data = bpy.data.cameras.new("ThumbCam")
    cam_data.type = 'PERSP'
    cam_data.lens = 50
    cam_obj = bpy.data.objects.new("ThumbCam", cam_data)
    cam_obj.location = (0, -2.4, 2.0)
    cam_obj.rotation_euler = (0.75, 0, 0)  # ~43 degrees down
...
def render_material(mat, scene, plane_obj, output_dir):
    ...
    if PREFIX_WITH_BLENDFILE:
        ...
        filename = f"{safe_name(blend_stem)}__{safe_name(mat.name)}.png"
    else:
        filename = f"{safe_name(mat.name)}.png"
